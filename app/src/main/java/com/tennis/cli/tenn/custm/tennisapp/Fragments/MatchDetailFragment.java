package com.tennis.cli.tenn.custm.tennisapp.Fragments;

import android.graphics.drawable.PictureDrawable;
import android.net.Uri;
import android.os.Bundle;

import androidx.appcompat.widget.AppCompatImageView;
import androidx.appcompat.widget.AppCompatTextView;
import androidx.fragment.app.Fragment;

import android.text.TextUtils;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.bumptech.glide.RequestBuilder;
import com.tennis.cli.tenn.custm.tennisapp.Activities.MainActivity;
import com.tennis.cli.tenn.custm.tennisapp.Application.TennisApp;
import com.tennis.cli.tenn.custm.tennisapp.Models.CModel.CountryModel;
import com.tennis.cli.tenn.custm.tennisapp.Models.DailySummeries.Summeraies;
import com.tennis.cli.tenn.custm.tennisapp.Models.PlayersModel;
import com.tennis.cli.tenn.custm.tennisapp.R;
import com.tennis.cli.tenn.custm.tennisapp.Utils.GlideApp;
import com.tennis.cli.tenn.custm.tennisapp.Utils.SvgSoftwareLayerSetter;

import java.util.List;

import de.hdodenhof.circleimageview.CircleImageView;


public class MatchDetailFragment extends Fragment {

    private View view;
    private Summeraies summeraies;
    private AppCompatImageView backImg;
    private String from;

    private CircleImageView flagOne,flagTwo,flagOneSp,flagTwoSp;

    private AppCompatTextView finalResultTxt,tNameTxt,couNametxt,citNameTxt,teamOrPlayerOneTxt,teamOrPlayerTwoTxt,playerOneTxt,playerTwoTxt,roundTxt,playerOne,playerTwo,playerOneSp,playerTwoSp;

    private RequestBuilder<PictureDrawable> requestBuilder;

    private String USAFLAG = "data:image/png;base64,";
    private String CREPFLAG = "data:image/jpeg;base64,";
    private String UKFLAG = "data:image/png;base64,";

    private String status = "";

    private AppCompatTextView bpo,bpt,dfo,dft,fsWo,fsWt,fsSo,fsSt,gwo,gwt,mgiro,mgirt,mpiro,mpirt,pwo,pwt,sspwo,sspwt,ssso,ssst,sgwo,sgwt,splo,splt,spwo,spwt,tbwo,tbwt;

    private String finalResultsString = "";

    public MatchDetailFragment() {
        // Required empty public constructor
    }


    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        view =  inflater.inflate(R.layout.fragment_match_detail, container, false);

        requestBuilder =
                GlideApp.with(getActivity())
                        .as(PictureDrawable.class)
                        .placeholder(R.drawable.ic_flag)
                        .listener(new SvgSoftwareLayerSetter());

        backImg = view.findViewById(R.id.back);

        summeraies = TennisApp.getSummeraies();

        status = summeraies.getSport_event_status().getStatus();

        from = TennisApp.getMatchDetailFrom();

        flagOne = view.findViewById(R.id.flag_img_one);
        flagTwo = view.findViewById(R.id.flag_img_two);
        flagOneSp = view.findViewById(R.id.flag_img_one_sp);
        flagTwoSp = view.findViewById(R.id.flag_img_two_sp);

        playerOne = view.findViewById(R.id.player_one);
        playerTwo = view.findViewById(R.id.player_two);
        playerOneSp = view.findViewById(R.id.player_one_sp_name);
        playerTwoSp = view.findViewById(R.id.player_two_sp_name);


        finalResultTxt = view.findViewById(R.id.final_results);
        tNameTxt = view.findViewById(R.id.tournament_name);
        couNametxt = view.findViewById(R.id.country_name);
        citNameTxt = view.findViewById(R.id.city_name);
        teamOrPlayerOneTxt = view.findViewById(R.id.team_or_plyr_one_txt);
        teamOrPlayerTwoTxt = view.findViewById(R.id.team_or_plyr_two_txt);
        playerOneTxt = view.findViewById(R.id.players_one_name);
        playerTwoTxt = view.findViewById(R.id.players_two_name);
        roundTxt = view.findViewById(R.id.round_name);

        // scores //
        bpo = view.findViewById(R.id.breaking_points_one);
        bpt = view.findViewById(R.id.breaking_points_two);

        dfo = view.findViewById(R.id.double_faults_one);
        dft = view.findViewById(R.id.double_faults_two);

        fsWo = view.findViewById(R.id.first_serve_won_one);
        fsWt = view.findViewById(R.id.first_serve_won_two);

        fsSo = view.findViewById(R.id.first_serve_succ_one);
        fsSt = view.findViewById(R.id.first_serve_succ_two);

        gwo = view.findViewById(R.id.games_won_one);
        gwt = view.findViewById(R.id.games_won_two);

        mgiro = view.findViewById(R.id.max_games_row_one);
        mgirt = view.findViewById(R.id.max_games_row_two);

        mpiro = view.findViewById(R.id.max_points_row_one);
        mpirt = view.findViewById(R.id.max_points_row_two);

        pwo = view.findViewById(R.id.points_won_one);
        pwt = view.findViewById(R.id.points_won_two);

        sspwo = view.findViewById(R.id.second_serve_won_one);
        sspwt = view.findViewById(R.id.second_serve_won_two);

        ssso = view.findViewById(R.id.second_serve_succ_one);
        ssst = view.findViewById(R.id.second_serve_succ_two);

        sgwo = view.findViewById(R.id.service_games_won_one);
        sgwt = view.findViewById(R.id.service_games_won_two);

        splo = view.findViewById(R.id.service_points_lost_one);
        splt = view.findViewById(R.id.service_points_lost_two);

        spwo = view.findViewById(R.id.service_points_won_one);
        spwt = view.findViewById(R.id.service_points_won_two);

        tbwo = view.findViewById(R.id.tiebreaks_won_one);
        tbwt = view.findViewById(R.id.tiebreaks_won_two);

        String type = summeraies.getSport_event().getSport_event_context().getCompetition().getType();

        if (!type.contentEquals("doubles")){

            flagOneSp.setVisibility(View.GONE);
            flagTwoSp.setVisibility(View.GONE);


            String flagurlOne = getFlagUrl(TennisApp.getFlagsList(),summeraies.getSport_event().getCompetitors().get(0).getCountry());
            String flagurlTwo = getFlagUrl(TennisApp.getFlagsList(),summeraies.getSport_event().getCompetitors().get(1).getCountry());


            System.out.println("FLAG URL ONE : " + flagurlOne);
            System.out.println("FLAG URL two : " + flagurlTwo);

            if (summeraies.getSport_event().getCompetitors().get(0).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagOne);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagOne);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagOne);

            }else{

                if (TextUtils.isEmpty(flagurlOne)){

                    flagOne.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlOne);
                    requestBuilder.load(uri).into(flagOne);

                }
            }



            if (summeraies.getSport_event().getCompetitors().get(1).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagTwo);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagTwo);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagTwo);

            }else{

                if (TextUtils.isEmpty(flagurlTwo)){

                    flagTwo.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlTwo);
                    requestBuilder.load(uri).into(flagTwo);

                }
            }



            if (summeraies.getSport_event().getCompetitors().get(0).getName().length() >= 10){

                String createdStrOne = summeraies.getSport_event().getCompetitors().get(0).getName().substring(0,9);
                playerOne.setText(createdStrOne + "..");

                String createdStrTwo = summeraies.getSport_event().getCompetitors().get(1).getName().substring(0,9);
                playerTwo.setText(createdStrTwo + "..");

            }else{

                playerOne.setText(summeraies.getSport_event().getCompetitors().get(0).getName());
                playerTwo.setText(summeraies.getSport_event().getCompetitors().get(1).getName());
            }

            if (summeraies.getSport_event_status().getWinner_id().contentEquals(summeraies.getSport_event().getCompetitors().get(0).getId())){
                playerOne.setTextColor(getActivity().getResources().getColor(R.color.teal_200));
            }else{
                playerOne.setTextColor(getActivity().getResources().getColor(R.color.orange_def));
            }

            if (summeraies.getSport_event_status().getWinner_id().contentEquals(summeraies.getSport_event().getCompetitors().get(1).getId())){
                playerTwo.setTextColor(getActivity().getResources().getColor(R.color.teal_200));
            }else{
                playerTwo.setTextColor(getActivity().getResources().getColor(R.color.orange_def));
            }


        }else{

            flagOneSp.setVisibility(View.VISIBLE);
            flagTwoSp.setVisibility(View.VISIBLE);

            String flagurlOne = getFlagUrl(TennisApp.getFlagsList(),
                    summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry());

            String flagurlTwo = getFlagUrl(TennisApp.getFlagsList(),
                    summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry());

            String flagurlOneSp= getFlagUrl(TennisApp.getFlagsList(),
                    summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry());

            String flagurlTwoSp = getFlagUrl(TennisApp.getFlagsList(),
                    summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry());


            if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagOne);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagOne);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagOne);

            }else{

                if (TextUtils.isEmpty(flagurlOne)){

                    flagOne.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlOne);
                    requestBuilder.load(uri).into(flagOne);

                }
            }



            if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagOneSp);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagOneSp);

            }else if (summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagOneSp);

            }else{

                if (TextUtils.isEmpty(flagurlOneSp)){

                    flagOneSp.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlOneSp);
                    requestBuilder.load(uri).into(flagOneSp);

                }
            }

            if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagTwo);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagTwo);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagTwo);

            }else{

                if (TextUtils.isEmpty(flagurlTwo)){

                    flagTwo.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlTwo);
                    requestBuilder.load(uri).into(flagTwo);

                }
            }


            if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry().contentEquals("USA")){

                GlideApp.with(getActivity()).load(USAFLAG).into(flagTwoSp);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry().contentEquals("Czech Republic")){

                GlideApp.with(getActivity()).load(CREPFLAG).into(flagTwoSp);

            }else if (summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry().contentEquals("Great Britain")){

                GlideApp.with(getActivity()).load(UKFLAG).into(flagTwoSp);

            }else{

                if (TextUtils.isEmpty(flagurlTwoSp)){

                    flagTwoSp.setImageDrawable(getActivity().getResources().getDrawable(R.drawable.ic_flag));

                }else{

                    Uri uri = Uri.parse(flagurlTwoSp);
                    requestBuilder.load(uri).into(flagTwoSp);

                }
            }


            if (summeraies.getSport_event().getCompetitors().get(0).getName().length() >= 10){

                String createdStrOne = summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getName().substring(0,9);
                playerOne.setText(createdStrOne + "..");

                String createdStrTwo = summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getName().substring(0,9);
                playerOneSp.setText(createdStrTwo + "..");

                String createdStrThree = summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getName().substring(0,9);
                playerTwo.setText(createdStrThree + "..");


                String createdStrFour = summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getName().substring(0,9);
                playerTwoSp.setText(createdStrFour + "..");

            }else{

                playerOne.setText(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getName());
                playerOneSp.setText(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getName());
                playerTwo.setText(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getName());
                playerTwoSp.setText(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getName());

            }


            if (summeraies.getSport_event_status().getWinner_id().contentEquals(summeraies.getSport_event().getCompetitors().get(0).getId())){
                playerOne.setTextColor(getActivity().getResources().getColor(R.color.teal_200));
                playerOneSp.setTextColor(getActivity().getResources().getColor(R.color.teal_200));

                playerTwo.setTextColor(getActivity().getResources().getColor(R.color.orange_def));
                playerTwoSp.setTextColor(getActivity().getResources().getColor(R.color.orange_def));

            }else{
                playerTwo.setTextColor(getActivity().getResources().getColor(R.color.teal_200));
                playerTwoSp.setTextColor(getActivity().getResources().getColor(R.color.teal_200));

                playerOne.setTextColor(getActivity().getResources().getColor(R.color.orange_def));
                playerOneSp.setTextColor(getActivity().getResources().getColor(R.color.orange_def));

            }


        }



        tNameTxt.setText(summeraies.getSport_event().getSport_event_context().getCompetition().getName());

        try {
            couNametxt.setText(summeraies.getSport_event().getVenue().getCountry_name());
        }catch (NullPointerException ex){
            ex.printStackTrace();
            couNametxt.setText("Not found");

        }

        try {
            citNameTxt.setText(summeraies.getSport_event().getVenue().getCity_name());
        }catch (NullPointerException ex){
            ex.printStackTrace();
            citNameTxt.setText("Not found");
        }

        try {
            roundTxt.setText(summeraies.getSport_event().getSport_event_context().getRound().getName());
        }catch (NullPointerException ex){
            ex.printStackTrace();
            roundTxt.setText("Not found");
        }


        try {

            playerOneTxt.setText(summeraies.getSport_event().getSport_event_context().getSeason().getStart_date());

        }catch (NullPointerException ex){
            ex.printStackTrace();
            playerOneTxt.setText("Not found");

        }

        System.out.println("STATUS IS: " + status);

        if (status.contentEquals("not_started")){

            // Do nothing //

        }else{

            if (summeraies.getStatistics() != null){

                bpo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getBreakpoints_won() + "");
                bpt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getBreakpoints_won() + "");

                dfo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getDouble_faults() + "");
                dft.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getDouble_faults() + "");

                fsWo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getFirst_serve_points_won() + "");
                fsWt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getFirst_serve_points_won() + "");

                fsSo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getFirst_serve_successful() + "");
                fsSt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getFirst_serve_successful() + "");

                gwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getGames_won() + "");
                gwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getGames_won() + "");


                mgiro.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getMax_games_in_a_row() + "");
                mgirt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getMax_games_in_a_row() + "");

                mpiro.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getMax_points_in_a_row() + "");
                mpirt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getMax_points_in_a_row() + "");

                pwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getPoints_won() + "");
                pwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getPoints_won() + "");

                sspwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getSecond_serve_points_won() + "");
                sspwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getSecond_serve_points_won() + "");

                ssso.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getSecond_serve_successful() + "");
                ssst.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getSecond_serve_successful() + "");

                sgwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getService_games_won() + "");
                sgwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getService_games_won() + "");

                splo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getService_points_lost() + "");
                splt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getService_points_lost() + "");

                spwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getService_points_won() + "");
                spwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getService_points_won() + "");

                tbwo.setText(summeraies.getStatistics().getTotals().getCompetitors().get(0).getStatistics().getTiebreaks_won() + "");
                tbwt.setText(summeraies.getStatistics().getTotals().getCompetitors().get(1).getStatistics().getTiebreaks_won() + "");


                for (int i = 0; i < summeraies.getSport_event_status().getPeriod_scores().size() ; i++){

                    if (finalResultsString.contentEquals("")){

                        finalResultsString = finalResultsString + summeraies.getSport_event_status().getPeriod_scores().get(i).getHome_score() + " - " + summeraies.getSport_event_status().getPeriod_scores().get(i).getAway_score();

                    }else{

                        finalResultsString = new StringBuilder().append(finalResultsString).append(" , ").append(summeraies.getSport_event_status().getPeriod_scores().get(i).getHome_score()).append(" - ").append(summeraies.getSport_event_status().getPeriod_scores().get(i).getAway_score()).toString();
                    }

                }


                System.out.println("FINAL RESULT: " + finalResultsString);
                finalResultTxt.setText(finalResultsString);
            }
        }

        backImg.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (from != null && from.contains("Rankings")){

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_nav_rankings);
                    }

                }else{

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_nav_tournaments);
                    }
                }
            }
        });


        flagOne.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (!type.contentEquals("doubles")){

                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }else{


                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }

            }
        });

        flagOneSp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {


                String maleOrFemale = "";
                String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                if (gen.contentEquals("men")){

                    maleOrFemale = "M";

                }else{

                    maleOrFemale = "F";

                }

                PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getId(),
                        "","",summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getName(),"","",
                        summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry(),maleOrFemale);

                TennisApp.setFrom("MD");
                TennisApp.setSelectedPlayerModel(playersModel);

                if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                    MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                }



            }
        });

        flagTwo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (!type.contentEquals("doubles")){

                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(1).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(1).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }else{


                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }

            }
        });

        flagTwoSp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {


                String maleOrFemale = "";
                String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                if (gen.contentEquals("men")){

                    maleOrFemale = "M";

                }else{

                    maleOrFemale = "F";

                }

                PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getId(),
                        "","",summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getName(),"","",
                        summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry(),maleOrFemale);

                TennisApp.setFrom("MD");
                TennisApp.setSelectedPlayerModel(playersModel);

                if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                    MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                }


            }
        });

        playerOne.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (!type.contentEquals("doubles")){

                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }else{


                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }

            }
        });

        playerOneSp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {


                String maleOrFemale = "";
                String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                if (gen.contentEquals("men")){

                    maleOrFemale = "M";

                }else{

                    maleOrFemale = "F";

                }

                PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getId(),
                        "","",summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getName(),"","",
                        summeraies.getSport_event().getCompetitors().get(0).getPlayers().get(1).getCountry(),maleOrFemale);

                TennisApp.setFrom("MD");
                TennisApp.setSelectedPlayerModel(playersModel);

                if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                    MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                }


            }
        });

        playerTwo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (!type.contentEquals("doubles")){

                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(1).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(1).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }else{


                    String maleOrFemale = "";
                    String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                    if (gen.contentEquals("men")){

                        maleOrFemale = "M";

                    }else{

                        maleOrFemale = "F";

                    }

                    PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getId(),
                            "","",summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getName(),"","",
                            summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(0).getCountry(),maleOrFemale);

                    TennisApp.setFrom("MD");
                    TennisApp.setSelectedPlayerModel(playersModel);

                    if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                        MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                    }

                }

            }
        });

        playerTwoSp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                String maleOrFemale = "";
                String gen = summeraies.getSport_event().getSport_event_context().getCompetition().getGender();

                if (gen.contentEquals("men")){

                    maleOrFemale = "M";

                }else{

                    maleOrFemale = "F";

                }

                PlayersModel playersModel = new PlayersModel(summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getId(),
                        "","",summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getName(),"","",
                        summeraies.getSport_event().getCompetitors().get(1).getPlayers().get(1).getCountry(),maleOrFemale);

                TennisApp.setFrom("MD");
                TennisApp.setSelectedPlayerModel(playersModel);

                if (MainActivity.getNavController() != null && MainActivity.getNavController().getCurrentDestination().getId() == R.id.matchDetailFragment){

                    MainActivity.getNavController().navigate(R.id.action_matchDetailFragment_to_profileFragment);
                }

            }
        });
        return view;
    }


    private String getFlagUrl(List<CountryModel> flagsList, String country) {

        String url = "";

        for (int i = 0; i < flagsList.size(); i++){

            if (flagsList.get(i).getName().contentEquals(country)){
                url = flagsList.get(i).getImage();
                return url;
            }
        }

        return url;
    }

}